...
class WikilocSpiderSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.wikiloc.com/directory/bjnYAg']   # link to place list WITHIN region
    ...
    def parse_town_page(self, response):
        ...
        if activities:
            ...
            trails = response.xpath('//ul[@class="geo-landing__body__canonical-trails__list"]/div[@class="figure__card__panel"]') #updated xpath 17/06/2025 
            next = response.xpath('//a[@rel="next"]/@href').extract_first() #updated xpath 17/06/2025      
            for t in trails:
                link = t.xpath('//a/@href').extract_first() #updated xpath 17/06/2025
                if link is not None:
                    item={}
