...
main_tree_plt <- ggplot(main_tree_data) +
  ...
  geom_fit_text(data = main_tree_data,
                ...
                #min.size = 4,
                reflow = TRUE) + 
  #labs(title = "Overall Consensus Category Breakdown") +
  scale_fill_manual(values = c("#F1AB4A", "#DCDCDC","#EFF171", 
                                "#ABE071", "#0F5726", "#FFFFFF","#B4B6BB")) +
  theme_void() +
...
# These correspond to the maps which agree with dominant class
class1_fnf_combo$fnf_combo_main = c("Hansen/IGBP, ESA, JAXA, CORINE, CORINE+LBM-DE",
                                    "Hansen/IGBP, ESA, JAXA, CORINE, FAO",
                                    "Hansen/IGBP, ESA, JAXA, CORINE+LBM-DE, FAO",
                                    "ESA, JAXA, CORINE, CORINE+LBM-DE, FAO",
                                    "Hansen/IGBP, ESA, CORINE, CORINE+LBM-DE, FAO",
                                    "Hansen/IGBP, JAXA, CORINE, CORINE+LBM-DE, FAO"
                                    ) 

# Add labels for sub category names
# These correspond to the maps which don't agree with dominant class
...
class1_tree_plt <- ggplot(class1_tree_data) +
  ...
  geom_fit_text(data = class1_tree_data,
                ...
                size = 8,
                reflow = TRUE) + 
  #labs(title = "Class 1 Breakdown: Which 5 maps agreee on non-forest (while 1 other indicates forest)?") +
  #scale_fill_manual(values = c("#F1AB4A", "#DCDCDC","#EFF171", 
  #                              "#ABE071", "#0F5726", "#FFFFFF","#B4B6BB")) +
  theme_void() +
  theme(legend.position = "none",
...
# Add labels for main category names
# These correspond to the maps which agree with dominant class
class2_fnf_combo$fnf_combo_main = c("Hansen/IGBP, ESA, JAXA, CORINE+LBM-DE",
                                    "Hansen/IGBP, ESA, JAXA, FAO",
                                    "ESA, JAXA, CORINE+LBM-DE, FAO",
                                    "Hansen/IGBP, JAXA, CORINE+LBM-DE, FAO",
                                    "Hansen/IGBP, JAXA, CORINE, CORINE+LBM-DE",
                                    "Hansen/IGBP, JAXA, CORINE, FAO",
                                    "JAXA, CORINE, CORINE+LBM-DE, FAO",
                                    "Hansen/IGBP, CORINE, CORINE+LBM-DE, FAO",
                                    "Hansen/IGBP, ESA, JAXA, CORINE",
                                    "ESA, JAXA, CORINE, FAO",
                                    "ESA, JAXA, CORINE, CORINE+LBM-DE",
                                    "Hansen/IGBP, ESA, CORINE+LBM-DE, FAO",
                                    "Hansen/IGBP, ESA, CORINE, CORINE+LBM-DE",
                                    "Hansen/IGBP, ESA, CORINE, FAO",
                                    "ESA, CORINE, CORINE + LBM-DE, FAO"
                                    ) 

# Add labels for sub category names
# These correspond to the maps which don't agree with dominant class
class2_fnf_combo$fnf_combo_sub = c("CORINE, FAO",
                                   "CORINE, CORINE+LBM-DE",
                                   "CORINE, Hansen/IGBP",
                                   "ESA, CORINE",
                                   "ESA, FAO",
                                   "ESA, CORINE+LBM-DE",
...
treemap(class2_fnf_combo,
        ...
        vSize="forest_area_ha",
        type="index",
        title = "Class 2 Breakdown: Which 4 maps agreee on non-forest (while 2 others indicate forest)?",
        fontsize.labels = 8,
        fontsize.title = 11,
...
treemap(class3_fnf_combo,
        ...
        vSize="forest_area_ha",
        type="index",
        title = "Class 3 Breakdown: Which 3 maps agree on forest (while 3 others indicate non-forest)?",
        fontsize.title = 11
        )
...
# Plot tree map
treemap(class4_fnf_combo,
        index= "fnf_combo",
        vSize="forest_area_ha",
        type="index",
        title = "Class 4 Breakdown: Which 4 maps agree on forest (while 2 others indicate non-forest)?",
        fontsize.title = 11
        )
...
# Tidy-up: remove the first column (index) and first row (old column names)
class5_fnf_combo <- class5_fnf_combo[2:16,2:4]
...
# Plot tree map
treemap(class5_fnf_combo,
        index= "fnf_combo",
        vSize="forest_area_ha",
        type="index",
        title = "Class 5 Breakdown: Which 5 maps agree on forest (while 1 other indicates non-forest)?",
        fontsize.title = 11
        )
...
row2 <- plot_grid(class1_tree_plt, NULL, labels = c('b', 'c'), label_size = 12, ncol = 2)
row3 <- plot_grid(NULL, NULL, labels = c('d', 'e'), label_size = 12, ncol = 2)

plot_grid(main_tree_plt, row2, row3,
          labels = c('a', '', ''),
          label_size = 12,
          #rel_widths = c(2, 1),
          #rel_heights = 4,
...
```{r all_tree_maps_save}

# Once happy with the outputs, save them!
# Note this will output whatever plot was run last!
ggsave("outputs/figures/tree_maps.png",
       width=7,
       height=10
       )
