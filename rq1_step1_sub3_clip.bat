...
for %%F in ("%working_folder%\*_reclass.tif") do (
    ...
    set "output_file=%working_folder%\!filename!_clipped_align.tif"

    :: Run gdalwarp with cutline 
    gdalwarp.exe -crop_to_cutline -cutline %shapefile% -tr 5 5 -tap -dstnodata -9999 -ot UInt16 -co COMPRESS=LZW -co BIGTIFF=YES %%F !output_file!
